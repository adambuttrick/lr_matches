...
# Remove country references used in the primary names of some ROR records for disambiguation
def preprocess_primary_name(name):
    name = re.sub(r'\s\(.*\)', '', name)
    return name
    

# Used to skip labels that have non-Latin characters (and thus cannot be used in pseudo affiliation strings)
...
# Create pseduo affiliation strings from the names and locations in the matches for use
# by the classifier
def format_matches(results):
    ...
    for record in records:
        ...
        if labels != []:
            labels = [label['label']
                      for label in labels if check_latin_chars(label['label'])]
        variants = aliases + labels + acronyms
        address = record['addresses'][0]
        city_name = address['geonames_city']['city'] if 'city' in address['geonames_city'].keys(
        ) else None
        admin1_name = address['geonames_city']['geonames_admin1']['name'] if 'geonames_admin1' in address['geonames_city'].keys(
        ) else None
        country_name = record['country']['country_name']
        location = ' '.join(
            [part for part in [city_name, admin1_name, country_name] if part != None])
        record_metadata = {'ror_id': ror_id, 'primary_name': primary_name,
                           'variant_names': variants, 'location': location}
...
def predict_ror_id_weighted(affiliation, matches):
    ...
    text_data = [normalize_text(affiliation, True)] + [normalize_text(
        m["primary_name"] + " " + " ".join(m["variant_names"]) + " " + m["location"]) for m in matches]
    print(text_data)
...
def parse_and_query(f):
    ...
    with open(outfile, 'w') as f_out:
        ...
        #adjust header to match your input file
        writer.writerow(['ror_id', 'doi', 'institution', 'affiliation',
                         'query_url', 'in_results', 'chosen', 'index', 'chosen_id'])
    with open(f, 'r+', encoding='utf-8-sig') as f_in:
        reader = csv.DictReader(f_in)
        for row in reader:
            affiliation = row['affiliation']
            assigned_ror_id = row['ror_id']
            if assigned_ror_id != '' and assigned_ror_id != None:
                query_url, ror_id_in_results, ror_id_chosen, index, chosen_id = query_affiliation(
                    affiliation, assigned_ror_id)
                with open(outfile, 'a') as f_out:
                    writer = csv.writer(f_out)
                    writer.writerow(list(
                        row.values()) + [query_url, ror_id_in_results, ror_id_chosen, index, chosen_id])
